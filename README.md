...
Ниже — **готовый блок для вставки в `README.md`** и **краткая инструкция, как создать символическую ссылку через PowerShell**.
Только по делу, без лишних пояснений.
...
В проекте используется встроенный (embedded) Python, расположенный в директории:

```

python-3.11.0-embed-amd64/python.exe

````

Для удобства работы создаётся **символическая ссылка** в корне проекта.
...
New-Item -ItemType SymbolicLink `
         -Path python.exe `
         -Target ".\python-3.11.0-embed-amd64\python.exe"
````
...
> или включённый **Developer Mode** в Windows.
...
или, если создана ссылка:

```powershell
.\py.exe script.py
```

⚠️ Команды `python` и `py` без `.\` запускают системный интерпретатор
или Python Launcher и **не используют embedded Python**.

```

---

### Кратко (вне README, для понимания)

- `New-Item -ItemType SymbolicLink` — нативный способ в PowerShell  
- `.\python.exe` — гарантированный запуск embedded Python  
- `python` / `py` — **не использовать**, если нужен embedded интерпретатор  
