...
**Только если Foundry уже запущен:**

```bash
# Если Foundry работает, можно запустить только FastAPI
python run.py
```
...
# 3. Запустить FastAPI (во втором терминале, если Foundry уже работает)
.\python.exe run.py
# или
.\py.exe run.py
...
# 4. Запустить FastAPI (второй терминал, если Foundry уже работает)
.\python.exe run.py
# или
.\py.exe run.py
